...
async function check(name, fn) {
  try {
    ...
    const vacuous = outcome.examined === 0 && !outcome.observation;
    results.push({ name, ...outcome, vacuous });
    const mark = outcome.observation ? "NOTE" : vacuous ? "VACUOUS" : outcome.ok ? "PASS" : "FAIL";
...
await check("no coupon has been redeemed past its global cap", async () => {
  ...
  return {
    ...
    examined: await countRows(`SELECT COUNT(*) AS n FROM coupons WHERE usage_limit IS NOT NULL`),
...
await check("how much of the redemption trail refers to a deleted coupon", async () => {
  /*
  ...
  */
  ...
  /*
    Reported, never failed. Deleting a coupon deliberately leaves its redemption
    trail behind, and that trail is the record of discounts real members were
    given — it must not be tidied away to make this line go green. What matters
    is that somebody reads the number, because it is also the reason the checks
    above have nothing to look at.
  */
  return {
    ok: true,
    observation: true,
    detail:
      total === 0 && orphanCounters === 0
        ? "none — every redemption still has its coupon"
        : `${total} redemptions and ${orphanCounters} counter rows outlive their coupon; the checks that join \`coupons\` cannot see them`,
    examined: await countRows(`SELECT COUNT(*) AS n FROM coupon_redemptions`),
...
await check(
  "every delivery item past `otp_sent` carries the timestamp that proves it",
  async () => {
    /*
...
const observations = results.filter((r) => r.observation);
const passed = results.filter((r) => r.ok && !r.vacuous && !r.unreadable && !r.observation);
say();
say(`## Summary`);
say();
...
say(`- observations (reported, not pass/fail): **${observations.length}**`);
