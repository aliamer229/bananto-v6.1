...
export function createBlankProductForm(defaultCategoryId: string): Record<string, any> {
  return {
    ...
    warrantyCondition: "",
    connectivity: "",
    // Schema-specific sections start empty. Seeding an unrelated amiibo,
    // accessory, gift-card or used-product example here persisted that example
    // into every imported game and made the details page show false facts.
    // The active schema/import is the only source allowed to fill these.
    characterName: "",
    amiiboSeries: "",
    figureType: "",
    inGameUnlock: "",
