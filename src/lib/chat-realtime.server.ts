...
/**
 * Cloudflare Durable Object class for live chat synchronization and fan-out.
 */
export class ChatRealtimeDO {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "setTyping" && request.method === "POST") {
      ...
      this.cleanExpired();
      const activeTypers = Array.from(this.typingMap.values()).map((t) => ({
        userId: t.userId,
        userName: t.userName,
        senderRole: t.senderRole,
      }));
    ...
    if (path === "isUserOnline") {
      ...
      const isOnline = !!p && Date.now() - p.lastSeen < 60_000;
  ...
  private cleanExpired() {
    const now = Date.now();
    for (const [uid, item] of this.typingMap.entries()) {
      if (item.expiresAt < now) {
