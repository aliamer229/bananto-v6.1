const fs = require('fs');
let code = fs.readFileSync('src/routes/category.$categoryId.tsx', 'utf-8');

const oldSort = `        case "newest":
        default: {
          const timeA = new Date(a.createdAt || 0).getTime();
          const timeB = new Date(b.createdAt || 0).getTime();
          return timeB - timeA;
        }`;

const newSort = `        case "newest":
        default: {
          const getVal = (p) => {
            if (p.releaseDate) return new Date(p.releaseDate).getTime();
            if (p.release_date) return new Date(p.release_date).getTime();
            if (p.metadata?.releaseDate) return new Date(p.metadata.releaseDate).getTime();
            if (p.metadata?.release_date) return new Date(p.metadata.release_date).getTime();
            return new Date(p.createdAt || 0).getTime();
          };
          return getVal(b) - getVal(a);
        }`;

code = code.replace(oldSort, newSort);
fs.writeFileSync('src/routes/category.$categoryId.tsx', code);
